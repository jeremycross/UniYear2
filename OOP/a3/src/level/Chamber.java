...
public class Chamber extends Space {
    ...
    private void formatExitsDescription(ArrayList<Door> doors, StringBuilder string) {
        for (int i = 0; i < doors.size(); i++) {
            ...
            if (i == (doors.size() - 1)) {
                string.append("Door ").append(i + 1).append(": ");
    ...
    private void getFormattedContentsDescription(StringBuilder string) {
        if (myContents.getDescription().contains("empty")) { //Empty chamber
            ...
            if (myContents.getDescription().contains("monster") && !(myContents.getDescription().contains("treasure"))) { //Monster only
                ...
                getMoreFormattedContentsDescription(string);
            }
        }
    }
